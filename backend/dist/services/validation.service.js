...
exports.validateBirthYearRequired = validateBirthYearRequired;
exports.validateNameCharacters = validateNameCharacters;
exports.validateUniquePerson = validateUniquePerson;
exports.validateNoSelfParent = validateNoSelfParent;
exports.assertMaxTwoBiologicalParents = assertMaxTwoBiologicalParents;
exports.validateBirthBeforeDeath = validateBirthBeforeDeath;
exports.validateParentAgeBeforeChild = validateParentAgeBeforeChild;
exports.validateChildNotOlderThanParent = validateChildNotOlderThanParent;
exports.validateLeapYear = validateLeapYear;
exports.validateBidirectionalRelationship = validateBidirectionalRelationship;
exports.validateDifferentGenderPartners = validateDifferentGenderPartners;
exports.validateDifferentGenderParents = validateDifferentGenderParents;
exports.assertNoCycleOnAssignParent = assertNoCycleOnAssignParent;
exports.validateSingleTreeMembership = validateSingleTreeMembership;
exports.validateNoCrossedBranches = validateNoCrossedBranches;
 ...
 */
function validatePersonName(givenNames, familyNames, displayName) {
 ...
 */
function validateBirthYearRequired(birthDate) {
 ...
 */
async function validateUniquePerson(treeId, givenNames, familyNames, displayName, birthDate, excludeId) {
    const fullName = displayName || [...(givenNames || []), ...(familyNames || [])].join(' ').trim();
    if (!fullName)
        return; // Si no hay nombre, no validar unicidad
    const query = {
        treeId,
    ...
    const existingPerson = await person_model_1.PersonModel.findOne(query);
    if (existingPerson) {
 ...
 */
function validateNoSelfParent(childId, parentId) {
    if (String(childId) === String(parentId)) {
        throw new Error('Una persona no puede ser su propio padre o madre');
    }
}
async function assertMaxTwoBiologicalParents(childId) {
    const child = await person_model_1.PersonModel.findOne({ _id: childId, softDeleted: { $ne: true } }).select('fatherId motherId');
    if (!child)
        throw new Error('Persona no encontrada');
    const parents = [child.fatherId, child.motherId].filter(Boolean);
    if (parents.length >= 2)
        throw new Error('Máximo 2 progenitores biológicos');
}
// ===== VALIDACIONES DE FECHAS =====
/**
 * Valida que la fecha de nacimiento sea anterior a la fecha de fallecimiento
 */
function validateBirthBeforeDeath(birthDate, deathDate) {
 ...
 */
function validateParentAgeBeforeChild(parentBirthDate, childBirthDate) {
 ...
 */
function validateChildNotOlderThanParent(parentBirthDate, childBirthDate) {
    if (!parentBirthDate || !childBirthDate)
        return;
    const parentYear = extractYear(parentBirthDate);
    const childYear = extractYear(childBirthDate);
 ...
 */
function validateLeapYear(dateStr) {
 ...
 */
async function validateBidirectionalRelationship(personAId, personBId, relationshipType) {
    const personA = await person_model_1.PersonModel.findOne({ _id: personAId, softDeleted: { $ne: true } });
    const personB = await person_model_1.PersonModel.findOne({ _id: personBId, softDeleted: { $ne: true } });
    if (!personA || !personB) {
 ...
 */
async function validateDifferentGenderPartners(personAId, personBId) {
    const personA = await person_model_1.PersonModel.findOne({ _id: personAId, softDeleted: { $ne: true } }).select('sex');
    const personB = await person_model_1.PersonModel.findOne({ _id: personBId, softDeleted: { $ne: true } }).select('sex');
    if (!personA || !personB) {
        throw new Error('Una o ambas personas no existen');
    }
 ...
 */
async function validateDifferentGenderParents(childId) {
    const child = await person_model_1.PersonModel.findOne({ _id: childId, softDeleted: { $ne: true } }).select('fatherId motherId');
    if (!child) {
        throw new Error('Persona no encontrada');
    }
    ...
    const [father, mother] = await Promise.all([
        person_model_1.PersonModel.findOne({ _id: child.fatherId, softDeleted: { $ne: true } }).select('sex'),
        person_model_1.PersonModel.findOne({ _id: child.motherId, softDeleted: { $ne: true } }).select('sex')
    ]);
    if (!father || !mother) {
...
async function assertNoCycleOnAssignParent(childId, candidateParentId) {
    ...
    while (queue.length && depth < 200) {
        ...
        const p = await person_model_1.PersonModel.findById(current).select('fatherId motherId');
        if (!p)
            continue;
        if (p.fatherId)
            queue.push(String(p.fatherId));
        if (p.motherId)
            queue.push(String(p.motherId));
        depth++;
    }
}
 ...
 */
async function validateSingleTreeMembership(personId, treeId) {
    const person = await person_model_1.PersonModel.findById(personId).select('treeId');
    if (!person)
        throw new Error('Persona no encontrada');
 ...
 */
async function validateNoCrossedBranches(treeId) {
    // Esta validación es compleja y requeriría un análisis más profundo del árbol
    // Por ahora, implementamos una validación básica
    const persons = await person_model_1.PersonModel.find({ treeId }).select('fatherId motherId partners');
 ...
 */
async function validatePersonData(personData, treeId, excludeId) {
    ...
    // Validar unicidad
    await validateUniquePerson(treeId, personData.givenNames, personData.familyNames, personData.displayName, personData.birthDate, excludeId);
    ...
    if (personData.fatherId) {
        ...
        const father = await person_model_1.PersonModel.findById(personData.fatherId).select('birth');
    ...
    if (personData.motherId) {
        ...
        const mother = await person_model_1.PersonModel.findById(personData.motherId).select('birth');
    ...
    if (personData.partners && personData.partners.length > 0) {
        const currentPerson = await person_model_1.PersonModel.findById(excludeId).select('sex');
        if (currentPerson) {
